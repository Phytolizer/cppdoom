...
const misc::Default defaults[] = {
    ...
    {"default_compatibility_level", &doomstat::default_compatibility_level, -1,
     -1, static_cast<int>(doomstat::CompLevel::MAX_COMPATIBILITY_LEVEL) - 1},
    {
        "realtic_clock_rate",
        &realtic_clock_rate,
        100,
    ...
    {"demo_insurance", &doomstat::default_demo_insurance,
     ...
     static_cast<int>(doomstat::DemoInsurance::Never),
     static_cast<int>(doomstat::DemoInsurance::DuringDemoRecording)},
...
void loadDefaultsFromFile(file::File& f)
{
  ...
  while (!f.eof())
  {
    ...
    if ((index == misc::DEF_BOOL || index == misc::DEF_INT) &&
            !get_if<int>(&parsedParm) ||
            ...
            !get_if<std::vector<std::string>>(&parsedParm))
    {
      ...
      spdlog::warn(
          "Type mismatch: key '{}' expected {}, received {}. Ignoring.", def,
          expectedType, receivedType);
      continue;
    }
    if (index == misc::DEF_BOOL &&
        (*get_if<int>(&parsedParm) < 0 || *get_if<int>(&parsedParm) > 1))
    {
    ...
    if (index == misc::DEF_INT)
    {
      ...
      if (belowMin || aboveMax)
      {
        ...
        spdlog::warn(
            "Range problem: key '{}' allows values in the range {}..{}, "
