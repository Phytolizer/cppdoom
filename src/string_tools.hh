...
template <> inline std::optional<uint8_t> parseString<uint8_t>(std::string_view s)
{
    std::stringstream ss{s.data()};
    int temp;
    ss >> temp;
    if (temp < std::numeric_limits<uint8_t>::min() || temp > std::numeric_limits<uint8_t>::max() ||
        std::to_string(temp) != s)
    {
        return {};
    }
    else
...
template <> inline bool parseString<uint8_t>(std::string_view s, gsl::not_null<uint8_t*> v)
{
    std::stringstream ss{s.data()};
    int temp;
    ss >> temp;
