...
std::string sys::doomExeDir()
{
    ...
    if (!base.has_value())
    {
        ...
        else
        {
            ...
            auto lastSlash = std::find_if(argMeta.argv0.rbegin(), argMeta.argv0.rend(),
...
std::optional<std::string> sys::findFile(std::string_view fname, std::string_view ext)
{
...
std::optional<std::string> sys::internal::findFileInternal(std::string_view fname, std::string_view ext)
{
    ...
    for (const auto& s : search)
    {
        ...
        std::string path = fmt::format("{}{}{}{}{}", dir, !string_tools::hasTrailingSlash(dir) ? "/" : "", sub,
