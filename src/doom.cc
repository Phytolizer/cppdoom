...
void setAffinityMask()
{
  ...
  if (sound::snd_midiplayer ==
      sound::midiplayers[static_cast<int>(sound::MidiPlayerName::Sdl)])
  {
  ...
  if (video::process_affinity_mask != 0)
  {
    ...
    if (errbuf == nullptr)
    {
      spdlog::info("setAffinityMask: manual affinity mask is {}",
                   video::process_affinity_mask);
    }
    else
    {
      spdlog::error("setAffinityMask: failed to set process affinity mask: {}",
                    errbuf);
    }
  }
}
...
void setProcessPriority()
{
  if (video::process_priority != 0)
  {
    ...
    if (errbuf == nullptr)
    {
      spdlog::info("setProcessPriority: priority for the process is {}",
                   video::process_priority);
    }
    else
    {
      spdlog::error(
          "setProcessPriority: failed to set priority for the process: {}",
          errbuf);
    }
  }
}
