...
class File
{
  ...
  public:
    ...
    template <typename T> void readToArr(gsl::not_null<std::vector<T>*> buffer) noexcept(false);
...
template <typename T> void file::File::readTo(gsl::not_null<T*> buffer) noexcept(false)
{
    ...
    if (nread != 1)
    {
        if (eof())
        {
            throw io::IoException(fmt::format("early end-of-file reading {} bytes", sizeof(T)));
        }
        else
        {
            throw io::IoException(fmt::format("error reading {} bytes: {}", sizeof(T), strerror(errno)));
        }
    }
}

template <typename T> void file::File::readToArr(gsl::not_null<std::vector<T>*> buffer) noexcept(false)
{
    ...
    if (nread != 1)
    {
        if (eof())
        {
            throw io::IoException{fmt::format("early end-of-file reading {} bytes", sizeof(T) * buffer->size())};
        }
        else
        {
            throw io::IoException{
                fmt::format("error reading {} bytes: {}", sizeof(T) * buffer->size(), strerror(errno))};
        }
    }
}
