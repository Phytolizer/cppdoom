...
std::vector<action::Action> deh::codeptr(static_cast<std::size_t>(info::StateEnum::NUMSTATES));
std::vector<std::string> deh::spriteNames(static_cast<std::size_t>(info::SpriteEnum::NUMSPRITES) + 1);
std::vector<std::string> deh::musicNames(static_cast<std::size_t>(info::Music::NUMMUSIC) + 1);
std::vector<std::string> deh::soundNames(static_cast<std::size_t>(info::Sfx::NUMSFX) + 1);
std::optional<int> deh::dehMaxSoul{};
std::optional<int> deh::dehMegaHealth{};
std::optional<int> deh::dehMaxHealth{};
std::vector<bool> deh::mobjInfoBits(static_cast<std::size_t>(info::MobjType::NUMMOBJTYPES), false);
...
void deh::buildBexTables()
{
    ...
    {
        deh::codeptr[i] = info::states.at(static_cast<info::StateEnum>(i)).action;
    }
    for (; i < static_cast<int>(info::StateEnum::NUMSTATES); i += 1)
    {
        info::State& state = info::states.at(static_cast<info::StateEnum>(i));
    ...
    deh::musicNames[0] = deh::musicNames[static_cast<size_t>(info::Music::NUMMUSIC)] = "";
...
void deh::applyCompatibility()
{
    ...
    if (doomstat::comp[static_cast<std::size_t>(doomstat::CompFlag::COMP_MAXHEALTH)])
    {
    ...
    {
        info::mobjinfo[static_cast<std::size_t>(info::MobjType::MT_SKULL)].flags |= info::MobjFlag::MF_COUNTKILL;
    }
    else
    {
        info::mobjinfo[static_cast<std::size_t>(info::MobjType::MT_SKULL)].flags &= ~info::MobjFlag::MF_COUNTKILL;
...
void deh::changeCompTranslucency()
{
    constexpr std::array predefined_translucency = {
        info::MobjType::MT_FIRE,        info::MobjType::MT_SMOKE,     info::MobjType::MT_FATSHOT,
        info::MobjType::MT_BRUISERSHOT, info::MobjType::MT_SPAWNFIRE, info::MobjType::MT_TROOPSHOT,
        info::MobjType::MT_HEADSHOT,    info::MobjType::MT_PLASMA,    info::MobjType::MT_BFG,
        info::MobjType::MT_ARACHPLAZ,   info::MobjType::MT_PUFF,      info::MobjType::MT_TFOG,
        info::MobjType::MT_IFOG,        info::MobjType::MT_MISC12,    info::MobjType::MT_INV,
        info::MobjType::MT_INS,         info::MobjType::MT_MEGA,
    };
    ...
    for (const auto& t : predefined_translucency)
    {
        if (!deh::mobjInfoBits[static_cast<std::size_t>(t)])
        {
            if (video::getMode() == arglex::VideoMode::ModeGL ||
                doomstat::comp[static_cast<std::size_t>(doomstat::CompFlag::COMP_TRANSLUCENCY)])
            {
                info::mobjinfo[static_cast<std::size_t>(t)].flags &= ~info::MobjFlag::MF_TRANSLUCENT;
            }
            else
            {
                info::mobjinfo[static_cast<std::size_t>(t)].flags |= info::MobjFlag::MF_TRANSLUCENT;
